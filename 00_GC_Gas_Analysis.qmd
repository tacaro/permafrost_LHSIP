...
peak_area_to_nmol <- function(gas, peak_area, limit = NA, force_zero = FALSE) {
  ...
  if (is.na(limit)) {
    ...
    nmol = case_when(
      gas == "CH4" ~ (peak_area * (CH4_slope + CH4_SE)) + CH4_intercept,
      gas == "CO2" ~ (peak_area * (CO2_slope + CO2_SE)) + CO2_intercept
    )
  } else if (limit == "lower") {
    nmol = case_when(
    gas == "CH4" ~ (peak_area * (CH4_slope - CH4_SE)) + CH4_intercept,
...
Fix the fact that entries with undetectable gas do not exist (as opposed to being zeroes). SRI data formats.

```{r}
missing_entries <- tribble(
  ~sample_id,          ~dt,   ~nmol_per_ml_g_t0,  ~diff_nmol_per_ml_g, ~gas,
  "p54m(-4C)7d",      0,     0, 0, "CH4",
  "p54m(-4C)7d",      0,     0, 0, "CO2",
  "p54m(-4C)30d",     0,     0, 0, "CH4",
  "p54m(-4C)30d",     0,     0, 0, "CO2",
  "p83m(-4C)7d",      0,     0, 0, "CH4",
  "p83m(-4C)7d",      0,     0, 0, "CO2",
  "p83m(12C)30d",     0,     0, 0, "CH4",
  "pUS(12C)30d",      0,     0, 0, "CH4"
) |> 
  mutate(
...
cowplot::save_plot(
  filename = "fig_output/p_gas_data.pdf",
  plot = p_combined,
  base_height = 8,
  base_width = 10
)
