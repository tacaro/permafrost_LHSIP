...
# Extraction Standards - FIXME after mapping 21:0 and 23:0
...
analytes_peak_table_w_ideal_calib <- 
  ...
  left_join(
    global_calibs_w_ranges |> iso_get_calibration_range() |> 
      ...
      select("folder", "standards", "d2H_calib", "max_d2H_vs_H2" = "max"),
    by = c("folder", "standards", "d2H_calib")
  ) |>
  # decide which calibration has the ideal range for each analysis
  mutate(
    .by = c("folder", "d2H_calib", "peak_id"),
    ideal_calib = 
      # if no calibrations have values this high, use the calibration with the highest d2H
      if(all(d2H_vs_H2 > max_d2H_vs_H2))
        { max_d2H_vs_H2 == max(max_d2H_vs_H2) }
      # otherwise (i.e. if the d2H below range max for any calib)
      # use the calibration whose max is closed to the value
      # (i.e. use the calibration with the smallest range)
      else 
        { max_d2H_vs_H2 == min(max_d2H_vs_H2[d2H_vs_H2 <= max_d2H_vs_H2]) }
  )

# safety check for number of ideal calibs (should be excactly 1 for each peak)
n_ideal_calibs <- analytes_peak_table_w_ideal_calib |> summarize(.by = c("folder", "d2H_calib", "peak_id"), n_ideal_calibs = sum(ideal_calib))
stopifnot(
  "each peak has to have exactly 1 ideal calibration" = all(n_ideal_calibs$n_ideal_calibs == 1L)
)
...
analytes_peak_table_final |>
  mutate(
    ...
    calibration_standards = sprintf("%s (%s)", standards, has_mem_correct),
    cal_d2H = ifelse(has_mem_correct, cal_d2H_yes_mem, cal_d2H_no_mem),
    cal_d2H_se = ifelse(has_mem_correct, cal_d2H_yes_mem, cal_d2H_se_no_mem),
    ...
    d2H_range = ifelse(has_mem_correct, d2H_range_yes_mem, d2H_range_no_mem) |>
      factor() |> fct_relevel("within", after = 1L)
  ) |>
  # sensible filters for what's possible
  filter(cal_d2H >= -1000, cal_d2H <= 42000, area_range != "below") |>
  ggplot() +
  aes(
    x = area2, y = cal_d2H, color = area_range, 
    shape = d2H_range # sprintf("%s (%s)", standards, has_mem_correct)
  ) +
  geom_point() +
  scale_x_log10() +
  facet_grid(standards ~ ., scales = "free_y") +
  labs(shape = "calibration standards\n(memory correction)")
```
