...
FID_21PC <- FID_samples |> 
  ...
  select(sample, `Peak Name`, mass_analyte_total_ng, core, temp, inc_time_d, fraction) |> 
...
Calculate extraction efficiency for *only F3s* by dividing initial 23PC quantity by the estimated ng based on peak area. We do not do this for F2s because 23PC is removed from F2s during the silica chromatography step. The mean extraction efficiency for F3s will be applied to the F2s, with the assumption being that phospholipids extract at similar efficiency to glycolipids.

```{r}
# 23:0 PC (Extraction Efficiency)
FID_23PC <- FID_samples |> 
  filter(fraction == "F3", `Peak Name` == "23:0") |> # focus only on F3s but apply this extraction efficieny to F2s
  select(sample, `Peak Name`, mass_analyte_total_ng, core, temp, inc_time_d, fraction) |> 
...
mean_extraction_efficiency <- FID_23PC |> 
  summarize(extraction_efficiency = mean(extraction_efficiency))
```
