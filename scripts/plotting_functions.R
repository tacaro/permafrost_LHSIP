...
#' @param use_var_names whether to use actual variable names or just x/y
...
generate_regression_fit_label <- function(df, formula, func = lm, signif = 2, include_r2 = TRUE, use_var_names = TRUE, terms_order_low_to_high = FALSE, ...) {
  require("dplyr")
  require("broom")
  require("latex2exp")
      ...
      coefs = map2(has_error, fit, ~if(!.x) { 
        ...
        coeffs %>%
          mutate(
            ...
            simple_x = paste0("x_", 1:n()),
            x_term = case_when(
              is.na(estimate) ~ sprintf("??\\cdot\\textit{%s}", ifelse(use_var_names, term, simple_x)),
              term == "(Intercept)" ~ format_with_signif(estimate, signif = !!signif, sci_as_latex = TRUE, include_plus = TRUE),
              TRUE ~ sprintf("%s\\cdot\\textit{%s}", format_with_signif(estimate, signif = signif, sci_as_latex = TRUE, include_plus = TRUE), 
                             ifelse(use_var_names, term, simple_x))
            )
          )
      } else { NULL }),
      y_term = sprintf("\\textit{%s}", if (use_var_names) rlang::as_label(rlang::f_lhs(!!formula_expr)) else "y"),
